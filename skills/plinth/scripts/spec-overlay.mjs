 ...
 *
 *   node spec-overlay.mjs <plinth-output.png> <device-id> <out.png> [padding]
 */
...
const [, , inputFile, deviceId, outFile, paddingArg] = process.argv;
const device = DEVICES[deviceId];
if (!inputFile || !device || !outFile) {
  console.error("usage: spec-overlay <plinth-output.png> <device-id> <out.png> [padding=48]");
  process.exit(2);
}
const padding = Number(paddingArg ?? 48);
...
const cr = contentRect(device, "standalone");
const rects = [];
...
add(`content (safe area) y=${cr.y}`, cr.x, cr.y, cr.width, cr.height, "#76ff03");
